...
// The sync cursor is the high-water mark of every `u` the client could have
// missed: `list.u` plus the largest item `u`. Both handlePoll and handleJoin
// hand this back so the next poll's `since` covers all writes — keep the rule
// in one place.
async function computeCursor (
  db: D1Database,
  listId: string,
  list: Pick<ListRow, 'u'>
): Promise<number> {
  ...
  return Math.max(list.u, maxItemU?.m ?? 0)
}
...
export async function handlePoll (
  ...
  const cursor = await computeCursor(db, listId, list)
